...
public final class SmpCoreCombatScreen extends SmpCoreMenuBase {
	...
	protected void init() {
		...
		addDoubleRaw(new ItemStack(Items.APPLE), "Immortality min health", "Minimum health to keep when immortality is enabled (1.0 = half a heart).", () -> config.combat.immortalityMinHealth, v -> config.combat.immortalityMinHealth = v, 0.0, 2048.0, "hp");
		addToggle(new ItemStack(Items.ENDER_PEARL), "Immortality allow void death", "If enabled, falling into the void can still kill you.", () -> config.combat.immortalityAllowVoidDeath, v -> config.combat.immortalityAllowVoidDeath = v);
